...
class SpyderCore:
    ...
    def __init__(self, reactive=False, n_threads=2, iterations=2, max_deep=20):
        ...
        self.thread = None
        if reactive:
            self.thread = Thread(target=self.mongo_agent.connect_to_eventsourcing, args=(self.mongo_agent, self.event_sourcer))
            self.thread.start()
    ...
    def analyze(self, link, deep=0, readedLinks=[]):
        if deep > self.max_deep:
            print("Max deep reached at ", deep)
            return link
...
spyderCore = SpyderCore(max_deep=1, reactive=True)
